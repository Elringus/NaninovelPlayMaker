    ...
    public class SetPlayMakerVariable : Command
    {
        ...
        public override UniTask ExecuteAsync (CancellationToken cancellationToken)
        {
            ...
            if (Assigned(ArrayIndex))
            {
                ...
                if (var is null) { LogErrorWithPosition($"PlayMaker global array variable with name `{VariableName}` not found."); return UniTask.CompletedTask; }
                var.Set(ArrayIndex, value);
            }
            else
            {
                var var = FsmVariables.GlobalVariables.FindVariable(VariableName);
                if (var is null) { LogErrorWithPosition($"PlayMaker global variable with name `{VariableName}` not found."); return UniTask.CompletedTask; }
                var.RawValue = value;
            }
